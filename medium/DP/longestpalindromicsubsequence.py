    ...
    def longestPalindromeSubseq(self, s: str) -> int:
        ...
        for i in range(0, strlen):
            ...
            for j in range(i-1, -1, -1):
                if s[i] == s[j]:
                    # if right == left, then longest from left to right == left+1 to right-1 + 2 (include both left and right)
                    dp[j][i] = dp[j+1][i-1] + 2
                else:
                    # else, longest from left to right == max(left+1 to right (include right), left to right - 1(include left))
